...
while getopts "buh:d:" opt; do
    case "$opt" in
    u)  
        echo -e "\nUsage: ./scripts/db/importDb.sh -h HOST -d DB. Defaults are host: localhost, database: safe. If -b flag, only import analytics and visualizationtypes.\n"
        exit 0
        ;;
    b)  importAllData=false
        ;;
    h)  host=$OPTARG
        ;;
    d)  db=$OPTARG
